...
# -*- coding: utf-8 -*-
"""
pretu_large_file_structure.py
大きなファイルのRead操作がブロックされた際にファイル構造を表示するhook

Note:
    pretu_large_file_check.py より後に実行される前提。
    ブロック判定自体は行わず、構造情報の付加のみを行う。
"""
...
try:
    ...
    sys.path.insert(0, str(Path(__file__).parent))  # pre_tool_use/ を追加
    from hook_utils import log_hook_execution, debug_log, get_path_from_config, log_hook_error, load_json5
    from file_structure_extractor import extract_file_structure, get_supported_extensions
...
def build_structure_message(file_path: Path, line_count: int, structure: str) -> str:
    """構造情報付きのブロックメッセージを生成"""
    return f"""[BLOCKED] 大きなファイル ({line_count} 行)

{file_path.name} の構造:
{structure}

offsetとlimitパラメータを指定して必要な部分のみ読み込んでください。"""


def execute(tool_data: dict):
    """
    大きなファイルのReadを検出してファイル構造を表示

    Args:
        tool_data: パッケージから渡されたツールデータ

    Note:
        構造情報がある場合のみブロック（stop_tool_use）する
        構造情報がない場合は何もしない（pretu_large_file_checkに任せる）
    """
    try:
        log_hook_execution()
        ...
        if not structure:
            # 構造が抽出できなければ何もしない（pretu_large_file_checkに任せる）
            return

        debug_log("started.")
        debug_log(f"BLOCKED with structure: {file_path.name} ({line_count} lines)")
        debug_log("finished.")

        # 構造情報付きでブロック
        from hook_utils import stop_tool_use
        stop_tool_use(build_structure_message(file_path, line_count, structure))
