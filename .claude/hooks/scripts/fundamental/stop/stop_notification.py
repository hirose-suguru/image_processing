...
windows-toastsベースの通知hookスクリプト（タスク完了時に発火）
...
# focus_wezterm.vbsのパス（notification/ディレクトリに配置されているものを参照）
# .vbsはwscript.exeで実行され、コンソールウィンドウが表示されない
FOCUS_SCRIPT = Path(__file__).parent.parent / "notification" / "focus_wezterm.vbs"
...
def main():
    ...
    try:
        ...
        if not get_hook_toggle("notification"):
            debug_log("stop_notification: Notification disabled by toggle")
            debug_log("finished.")
            return
        ...
        try:
                ...
                try:
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            # debug_log(f"stop_notification: wezterm cli found {len(tabs)} tabs")
                            ...
                            if pane_id_str:
                                ...
                                for tab in tabs:
                                    if tab.get("pane_id") == pane_id:
                                        ...
                                        # debug_log(f"stop_notification: WEZTERM_PANE={pane_id} -> window_id={window_id}")
                                        break
                            # else:
                            #     debug_log("stop_notification: WEZTERM_PANE not set, skipping")
                        else:
                            debug_log(f"stop_notification: wezterm cli failed: {result.stderr.decode()[:100]}")
                    except Exception as e:
                        debug_log(f"stop_notification: wezterm cli error: {e}")
                    ...
                    # debug_log(f"stop_notification: tab_counts (excluding nvim): {tab_counts}")
                    ...
                    # debug_log(f"stop_notification: found {len(wezterm_windows)} wezterm windows")

                    # 2c. タブ数でマッチング
                    if window_id is not None and window_id in tab_counts:
                        target_tab_count = tab_counts[window_id]
                        # debug_log(f"stop_notification: target window_id={window_id}, tab_count={target_tab_count}")
                        ...
                        if len(matched_hwnds) == 1:
                            ...
                            # debug_log(f"stop_notification: matched by tab count: hwnd={hwnd}")
                        elif len(matched_hwnds) > 1:
                            # 複数マッチした場合は最初のものを使用
                            hwnd = matched_hwnds[0][0]
                            debug_log(f"stop_notification: multiple matches ({len(matched_hwnds)}), using first hwnd={hwnd}")
                        else:
                            debug_log(f"stop_notification: no match by tab count")

                    # フォールバック: マッチしなかった場合
                    if hwnd is None and len(wezterm_windows) == 1:
                        hwnd = wezterm_windows[0][0]
                        # debug_log(f"stop_notification: fallback to single window: hwnd={hwnd}")
                        ...
                        for h, title, _ in wezterm_windows:
                            if not any(nvim_title in title.lower() for nvim_title in NVIM_TITLES):
                                hwnd = h
                                debug_log(f"stop_notification: fallback (excluding nvim): hwnd={hwnd}")
                                break
                        if hwnd is None:
                            hwnd = wezterm_windows[0][0]
                            debug_log(f"stop_notification: fallback to first window: hwnd={hwnd}")
                except ImportError:
                    debug_log("stop_notification: pywin32 not available, skipping hwnd capture")
                except Exception as e:
                    debug_log(f"stop_notification: failed to get hwnd: {e}")
            ...
            # debug_log(f"stop_notification: saved cache: {cache_data}")
        except Exception as e:
            debug_log(f"stop_notification: failed to save cache: {e}")
        ...
        debug_log(f"stop_notification: toast={result['toast']}, tts={result['tts']}")
        debug_log("finished.")
